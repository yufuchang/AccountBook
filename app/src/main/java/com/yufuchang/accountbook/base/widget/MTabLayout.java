package com.yufuchang.accountbook.base.widget;
...
public class MTabLayout extends HorizontalScrollView {
    ...
    /**
     * Gravity used to fill the {@link MTabLayout} as much as possible. This option only takes effect
    ...
    /**
     * Gravity used to lay out the tabs in the center of the {@link MTabLayout}.
     *
     * @see #setTabGravity(int)
     * @see #getTabGravity()
     */
    ...
    public MTabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mTabStrip.setSelectedIndicatorWidth(
                a.getDimensionPixelSize(R.styleable.MTabLayout_mTabIndicatorWidth, -1));
        mTabStrip.setSelectedIndicatorColor(a.getColor(R.styleable.MTabLayout_mTabIndicatorColor, 0));

        mTabPaddingStart = mTabPaddingTop = mTabPaddingEnd = mTabPaddingBottom = a
                .getDimensionPixelSize(R.styleable.MTabLayout_mTabPadding, 0);
        mTabPaddingStart = a.getDimensionPixelSize(R.styleable.MTabLayout_mTabPaddingStart,
                mTabPaddingStart);
        mTabPaddingTop = a.getDimensionPixelSize(R.styleable.MTabLayout_mTabPaddingTop,
                mTabPaddingTop);
        mTabPaddingEnd = a.getDimensionPixelSize(R.styleable.MTabLayout_mTabPaddingEnd,
                mTabPaddingEnd);
        mTabPaddingBottom = a.getDimensionPixelSize(R.styleable.MTabLayout_mTabPaddingBottom,
                mTabPaddingBottom);
        ...
        mRequestedTabMinWidth = a.getDimensionPixelSize(R.styleable.MTabLayout_mTabMinWidth,
                INVALID_WIDTH);
        mRequestedTabMaxWidth = a.getDimensionPixelSize(R.styleable.MTabLayout_mTabMaxWidth,
                INVALID_WIDTH);
        mTabBackgroundResId = a.getResourceId(R.styleable.MTabLayout_mTabBackground, 0);
     ...
     * @attr ref R.styleable#MTabLayout_mTabIndicatorColor
     */
     ...
     * @attr ref R.styleable#MTabLayout_mTabIndicatorHeight
     */
    ...
    private void addTabFromItemView(@NonNull MTabItem item) {
     ...
     * @attr ref R.styleable#MTabLayout_mTabMode
     */
    ...
    /**
     * Returns the current mode used by this {@link MTabLayout}.
     *
     * @see #setTabMode(int)
     */
    @Mode
     ...
     * @attr ref R.styleable#MTabLayout_mTabGravity
     */
     ...
     * Sets the text colors for the different states (normal, selected) used for the tabs.
     *
     * @attr ref R.styleable#MTabLayout_mTabTextColor
     * @attr ref R.styleable#MTabLayout_mTabSelectedTextColor
     */
    ...
    /**
     * The one-stop shop for setting up this {@link MTabLayout} with a {@link ViewPager}.
     *
    ...
    /**
     * @deprecated Use {@link #setupWithViewPager(ViewPager)} to link a MTabLayout with a ViewPager
     * together. When that method is used, the MTabLayout will be automatically updated
    ...
    private void addViewInternal(final View child) {
        if (child instanceof MTabItem) {
            addTabFromItemView((MTabItem) child);
        } else {
    ...
    private class SlidingTabStrip extends LinearLayout {
        ...
        private void updateIndicatorPosition() {
            ...
            if (selectedTitle != null && selectedTitle.getWidth() > 0) {
                ...
                if (mSelectedIndicatorWidth != -1 && right - left > mSelectedIndicatorWidth) {
                    int cut = (right - left - mSelectedIndicatorWidth) / 2;
                    left = left + cut;
                    right = right - cut;
                }
                ...
                if (mSelectionOffset > 0f && mSelectedPosition < getChildCount() - 1) {
                    ...
                    if (mSelectedIndicatorWidth != -1 && nextTitleRight - nextTitleLeft > mSelectedIndicatorWidth) {
                        int cut = (nextTitleRight - nextTitleLeft - mSelectedIndicatorWidth) / 2;
                        nextTitleLeft = nextTitleLeft + cut;
                        nextTitleRight = nextTitleRight - cut;
                    }

//                    left = (int) (mSelectionOffset * nextTitleLeft + (1.0f - mSelectionOffset) * left);
//                    right = (int) (mSelectionOffset * nextTitleRight + (1.0f - mSelectionOffset) * right);

                    if (mSelectionOffset <= 0.5F) {
                        right = (int) (right + (nextTitleRight - right) * mSelectionOffset * 2F);
                    } else {
                        left = (int) (left + (nextTitleLeft - left) * (mSelectionOffset - 0.5F) * 2F);
        ...
        void animateIndicatorToPosition(final int position, int duration) {
            ...
            int cut = 0;
            if (mSelectedIndicatorWidth != -1 && targetView.getWidth() > mSelectedIndicatorWidth) {
                cut = (targetView.getWidth() - mSelectedIndicatorWidth) / 2;
            }

            final int targetLeft = targetView.getLeft() + cut;
            final int targetRight = targetView.getRight() - cut;
                ...
                animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                    ...
                    public void onAnimationUpdate(ValueAnimator animator) {
                        ...
                        if (startLeft < targetLeft) {
                            if (fraction <= 0.5F) {
                                setIndicatorPosition(startLeft, (int) (startRight + (targetRight - startRight) * fraction * 2F));
                            } else {
                                setIndicatorPosition((int) (startLeft + (targetLeft - startLeft) * (fraction - 0.5F) * 2F), targetRight);
                            }
                        } else {
                            if (fraction <= 0.5F) {
                                setIndicatorPosition((int) (startLeft - (startLeft - targetLeft) * fraction * 2F), startRight);
                            } else {
                                setIndicatorPosition(targetLeft, (int) (startRight - (startRight - targetRight) * (fraction - 0.5F) * 2F));
                            }
                        }
...
//                                lerp(startRight, targetRight, fraction));
        ...
        public void draw(Canvas canvas) {
            ...
            if (mIndicatorLeft >= 0 && mIndicatorRight > mIndicatorLeft) {
                mIndicatorRectF.set(mIndicatorLeft, getHeight() - mSelectedIndicatorHeight, mIndicatorRight, getHeight());
                canvas.drawRoundRect(mIndicatorRectF, mSelectedIndicatorHeight, mSelectedIndicatorHeight, mSelectedIndicatorPaint);
            }
        }
    }
     ...
     * necessary calls back to the provided {@link MTabLayout} so that the tab position is
     * kept in sync.
     *
     * <p>This class stores the provided MTabLayout weakly, meaning that you can use
     * {@link ViewPager#addOnPageChangeListener(ViewPager.OnPageChangeListener)
     * addOnPageChangeListener(OnPageChangeListener)} without removing the listener and
     * not cause a leak.
     */
    public static class TabLayoutOnPageChangeListener implements ViewPager.OnPageChangeListener {
        private final WeakReference<MTabLayout> mTabLayoutRef;
        private int mPreviousScrollState;
        private int mScrollState;

        public TabLayoutOnPageChangeListener(MTabLayout tabLayout) {
        ...
        public void onPageScrolled(final int position, final float positionOffset,
            ...
            final MTabLayout tabLayout = mTabLayoutRef.get();
            if (tabLayout != null) {
